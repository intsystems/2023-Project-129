...
We use \(f(t) = \sin(\pi t) + np.sin(et)\) for \(t \in (0, 100)\), sampling it to \(100\) points to create a time series. We then applied SSA with different numbers of components taken at reconstruction step. The data generated looks as follows:

\includegraphics[scale=0.7]{./images/fig1.png}

The actual results for log-MSE error for SSA with window size 5 are:
